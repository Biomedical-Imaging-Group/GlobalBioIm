...
if ~isPos && all(isdiff)
    ...
    listOpti{nOpti}.call{2} = 'Opt = OptiGradDsct(cf,gam);';
...
if all(isdiff)
    ...
    listOpti{nOpti}.parameters{1}.default = '0';
    listOpti{nOpti}.parameters{1}.info = 'Descent step';
    listOpti{nOpti}.parameters{2}.name = 'useFISTA';
...
if all(isdiff)
    ...
    listOpti{nOpti}.call{1} = ['cf = ',GetExprCompCost(NameCosts,NamesOps,find(isdiff))];
    if isPos
        listOpti{nOpti}.call{2} = 'VMLMB=OptiVMLMB(cf,0,Inf)';
    else
        listOpti{nOpti}.call{2} = 'VMLMB=OptiVMLMB(cf)';
    end
end
