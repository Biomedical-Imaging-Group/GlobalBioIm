classdef Fresnel <  LinOp
    ...
    methods
        function this = Fresnel(lambda, n0, z,dxy,sz, pad, varargin)
            ...
            assert((~isempty(pad))&& issize(pad)&& length(pad)~=2,'The padding pad should be a conformable  to size(2D)');
            this.pad = pad;
            ...
            if this.FeitFleck
                ...
                this.F =  exp(-2i* pi * this.lambda * Mesh ./ (1 + sqrt(1 + (this.lambda/this.n0)^2 *Mesh)));
            else
