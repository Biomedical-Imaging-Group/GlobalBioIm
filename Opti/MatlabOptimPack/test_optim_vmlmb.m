 % Matlab function similar to the C program test_optim_vmlmb.c
 % to test the Matlab interface to the OptimPack optimisation Package
 % for the unconstrained minimization of the rosenbrock function
 %
 ...
 % 
 % The results are identical to the test_optim_vmlmb.c ones.
  ...
  n=length(x_init); m=3; fatol=0.0; frtol=1e-8; sftol=0.001; sgtol=0.9; sxtol=0.1; epsilon=0.01; costheta=0.4;
  ...
  % Initialization
  x(1:n) = x_init; x=x(:); % Warning : side effect on x_init if x=x_init (due to the fact that x is passed-by-reference in the mexfiles)
  ...
  for iter=1:nbitermax
      ...
      % Computes next step:
      [task, csave]= m_vmlmb_next(x,f,g,[],isave,dsave);
      list_x(:,iter) = x;
      list_J(iter) = f;
  ...
  plot(list_x(1,:),list_x(2,:),'*-r',x(1),x(2),'ow'); hold off
  ...
  %% Test with call to the optim_vmlmb.m function
   ...
   xmin = [];
   xmax = [];
   option = [3; 0.0; 1e-8; 0.001; 0.9; 0.1; 0.01; 0.4; nbitermax; 2];

   x_init = [ -1 ; 1];
   fprintf('Minimization with VMLMB\n')
   fprintf('Initialization x= [%f ; %f]\n',x(1),x(2));
   [x, list_J , list_x] = optim_vmlmb(fun,grad_fun,x_init,xmin,xmax,option);
   g = grad_fun(x);
   fprintf('function = %.8f, grad = [%.8f ; %.8f]\n' , f, g(1), g(2));
   fprintf('Solution : x= [%.8f ; %.8f]\n', x(1),x(2));
