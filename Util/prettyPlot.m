function im_h = prettyPlot(xStart, xStep, xSize, f, labels, varargin)
% function h = prettyPlot(xStart, xStep, xSize, f, [labels])
%
% attractive plotting for 2d and 3d images
%
% assumes that the rows of f correspond to x_0 and the columns to x_1,
...
elseif ndims(f) == 3
	...
	fig_h(1) = figure;
	im_h(1) = prettyPlot(xStart(2:3), xStep(2:3), xSize(2:3), squeeze(f(sliceInd(1), :, :)), labels(2:3), varargin{:});
	axis_h(1) = gca;
	update_lines(im_h(1))
	
	fig_h(2) = figure;
	im_h(2) = prettyPlot(xStart([1 3]), xStep([1 3]), xSize([1 3]), squeeze(f(:,sliceInd(2), :)), labels([1 3]), varargin{:} );
	axis_h(2) = gca;
	update_lines(im_h(2))
	
	fig_h(3) = figure;
	im_h(3) = prettyPlot(xStart(1:2), xStep(1:2), xSize(1:2), squeeze(f(:,:,sliceInd(3))), labels(1:2), varargin{:});
	axis_h(3) = gca;
	update_lines(im_h(3))
	...
	function update_center(src, ~)
		...
		centerPoint = round(centerPoint ./ xStep) .* xStep;
